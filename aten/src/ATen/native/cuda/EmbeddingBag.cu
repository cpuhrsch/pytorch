...
__global__ void
EmbeddingBag_updateOutputKernel(int64_t *input, int64_t *offsets, scalar_t *weight,
                                scalar_t *output, int64_t *offset2bag,
                                int64_t numIndices, int64_t numBags,
  ...
  for (int64_t chunk = chunkOffset; chunk < numChunks; chunk += chunkStride) {
    ...
    if (featureDim < stride) {
      ...
      int64_t end =
          (bag < numBags - 1) ? (offsets[bag + 1]) : numIndices;
      assert(end >= begin);
      ...
      for (int64_t emb = begin; emb < end; emb++) {
        ...
        weightFeatSum +=
            scalar_cast<accscalar_t>(weightFeat[weightRow]);
        bag_size_++;
        if (featureDim == 0) {
      ...
      if (mode == MODE_MEAN) {
        weightFeatSum =
            weightFeatSum / scalar_cast<accscalar_t>(bag_size_);
        bag_size[bag] = bag_size_;
      }
      (void)MODE_SUM; // silence warnings about unused MODE_SUM;
      output[bag * stride + featureDim] =
          scalar_cast<scalar_t>(weightFeatSum);
    }
  }
}
...
__global__ void EmbeddingBag_accGradParametersKernel(
    int64_t *input, int64_t *indices, scalar_t *gradOutput, scalar_t *gradWeight,
...
#pragma unroll
      for (int ii = 0; ii < SZ; ii++) {
        ...
        if (featureDim < stride) {
          gradient[ii] = scalar_cast<accscalar_t>(
              gradOutput[gradOutputRow + featureDim]);
          if (mode == MODE_MEAN) {
            gradient[ii] /= bag_size[seq_number];
          }
          weight[ii] = scalar_cast<accscalar_t>(
...
Tensor embedding_bag_cuda(const Tensor &weight, const Tensor &indices,
                          const Tensor &offsets, const Tensor& offset2bag, const bool scale_grad_by_freq,
                          const int64_t mode) {
  ...
  auto bag_size = indices.type().tensor(offsets.sizes());
  ...
  DISPATCH_ALL_FLOATING_TYPES(weight.type(), "embedding_bag_cuda", [&]() {
    EmbeddingBag_updateOutputKernel<
...
// void THNN_(EmbeddingBag_accGradParameters)(
//            THCState *state,
...
Tensor embedding_bag_backward_cuda(const Tensor &grad_, const Tensor &indices,
                                   const Tensor &offsets, const Tensor& offset2bag, int64_t num_weights,
...
// std::cerr << "STARTING BACKWARD GPU" << std::endl;
...
// std::cerr << "offset2bag: " << offset2bag << std::endl;
  ...
  // THCIndexTensor_(nDimension)(state, input) != 2) {
  //   THCDescBuff s1 = THCIndexTensor_(sizeDesc)(state, indices);
  //   THError("indices must be a vector or matrix, but is of shape: %s",
  //   s1.str);
  // }
  ...
  if (mode == 1) { // MODE_MEAN
    if (offsets.sizes()[0] == 1) {
       bag_size[0] = indices.sizes()[0];
       // std::cerr << "scalar bag size: " << bag_size << std::endl;
    } else {
      bag_size = offsets.type().tensor(offsets.sizes());
      ...
      bag_size = bag_size.unsqueeze(1).index_select(0, offset2bag);
       ...
       // std::cerr << "grad_weight: " << grad_weight << std::endl;
  ...
  DISPATCH_ALL_FLOATING_TYPES(
      grad.type(), "embedding_bag_backward_cuda", [&]() {
        EmbeddingBag_accGradParametersKernel<
            ...
            count.defined() ? count.data<int64_t>() : nullptr, numel,
            stride, mode, bag_size.data<int64_t>());
      });
