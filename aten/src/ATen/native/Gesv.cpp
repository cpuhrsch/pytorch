...
static void applyGesv(Tensor& b, Tensor& A, std::vector<int64_t> infos) {
  ...
  for (int64_t i = 0; i < batch_size; i++) {
    ...
    lapackGesv<scalar_t>(
        n, nrhs, A_working_ptr, n, ipiv.data<int>(), b_working_ptr, n, &info);
    infos[i] = info;
    if (info != 0) {
      return;
    }
...
std::tuple<Tensor, Tensor> _gesv_helper_cpu(
    const Tensor& self,
    const Tensor& A) {
...
std::tuple<Tensor, Tensor> gesv(const Tensor& self, const Tensor& A) {
  ...
  self_expand_size.insert(
      self_expand_size.end(), {self.size(-2), self.size(-1)});
