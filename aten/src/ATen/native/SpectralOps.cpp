...
static inline Tensor _fft(
  ...
  auto signal_tensor_ndim =
      signal_ndim + static_cast<int>(complex_input); // add complex dim
  ...
  std::vector<int64_t> output_sizes(
      signal_ndim + 1 + static_cast<int>(complex_output));
  ...
  if (batch_ndim == 0) {
    ...
    auto output_ndim = self.dim() + static_cast<int>(complex_output) -
        static_cast<int>(complex_input);
...
#define REPR(SS)                                                   \
  SS << "stft(" << self.type() << self.sizes()                     \
     << ", frame_length=" << frame_length << ", hop=" << hop       \
     << ", fft_size=" << fft_size << ", normalized=" << normalized \
     << ", onesided=" << onesided << ", window=";                  \
  if (window.defined()) {                                          \
  ...
  if (window.defined() &&
      (window.dim() != 1 || window.size(0) != frame_length)) {
    std::ostringstream ss;
    REPR(ss) << ": expected a 1D window tensor of size equal to "
             << "frame_length=" << frame_length << ", but got window with size "
             << window.sizes();
    throw std::runtime_error(ss.str());
  }
#undef REPR
  int64_t return_size =
      onesided ? infer_ft_real_to_complex_onesided_size(fft_size) : fft_size;
  // build ft kernel
  // k[omega, t] = cos (2 pi omega t / N) - j sin (2 pi omega t / N)
  double N = static_cast<double>(fft_size);
  auto freq_arange =
      at::arange(self.type(), 0, return_size).mul_(M_PI * 2. / N);
