...
template <>
void sspaddmm_TH_dispatch<float>(
    Tensor& result,
    Scalar beta,
    const Tensor& self,
    Scalar alpha,
    const Tensor& mat1,
    const Tensor& mat2) {
  auto result_ = checked_cast_tensor<SparseCPUFloatTensor>(
      result.pImpl, "result", 0, false);
  auto beta_ = beta.toFloat();
  auto self_ =
      checked_cast_tensor<SparseCPUFloatTensor>(self.pImpl, "self", 2, false);
  auto alpha_ = alpha.toFloat();
  auto mat1_ =
      checked_cast_tensor<SparseCPUFloatTensor>(mat1.pImpl, "mat1", 4, false);
  auto mat2_ =
      checked_cast_tensor<CPUFloatTensor>(mat2.pImpl, "mat2", 5, false);
  THSFloatTensor_sspaddmm(
      result_->tensor,
      beta_,
      self_->tensor,
      alpha_,
      ...
      mat2_->tensor);
  result_->maybeScalar(
      self_->isScalar() && mat1_->isScalar() && mat2_->isScalar());
}

template <>
void sspaddmm_TH_dispatch<double>(
    Tensor& result,
    Scalar beta,
    const Tensor& self,
    Scalar alpha,
    const Tensor& mat1,
    const Tensor& mat2) {
  auto result_ = checked_cast_tensor<SparseCPUDoubleTensor>(
      result.pImpl, "result", 0, false);
  auto beta_ = beta.toDouble();
  auto self_ =
      checked_cast_tensor<SparseCPUDoubleTensor>(self.pImpl, "self", 2, false);
  auto alpha_ = alpha.toDouble();
  auto mat1_ =
      checked_cast_tensor<SparseCPUDoubleTensor>(mat1.pImpl, "mat1", 4, false);
  auto mat2_ =
      checked_cast_tensor<CPUDoubleTensor>(mat2.pImpl, "mat2", 5, false);
  THSDoubleTensor_sspaddmm(
      result_->tensor,
      beta_,
      self_->tensor,
      alpha_,
