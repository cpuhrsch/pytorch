...
static inline Tensor integer_upcast(
  ...
  ScalarType upcast_scalarType = dtype.value_or(
      at::isIntegralType(scalarType) ? ScalarType::Long : scalarType);
...
Tensor& cumprod_out(
  ...
  return at::native::cumprod_out(
      result, self, dim, optional<ScalarType>(dtype));
}
...
static bool _dimreduce_return_trivial(
    Tensor& result,
    const Tensor& self,
    int64_t ident) {
...
static Tensor& _dimreduce_setup(
    Tensor& result,
    const Tensor& self,
    int64_t dim) {
...
Tensor& sum_out(
  ...
  return at::native::sum_out(
      result, self, dim, keepdim, at::optional<ScalarType>(dtype));
}
...
Tensor& logsumexp_out(
  ...
  result = at::where(
      (maxes == INFINITY).__or__(maxes == -INFINITY),
      maxes,
...
inline Tensor reduce_multi_associative(
    const Tensor& self,
    IntList dims_,
    bool keepdim) {
  if (dims_.size() == 1) {
...
template <
    Tensor(reduce_1)(const Tensor&, int64_t, bool),
    Tensor&(reduce_1_out)(Tensor& result, const Tensor&, int64_t, bool)>
inline Tensor& reduce_multi_associative_out(
    Tensor& result,
    const Tensor& self,
    IntList dims_,
    bool keepdim) {
...
Tensor& _sum_out(
    ...
    IntList dims,
    bool keepdim) {
  return reduce_multi_associative_out<_sum, _sum_out>(
      result, self, dims, keepdim);
}
