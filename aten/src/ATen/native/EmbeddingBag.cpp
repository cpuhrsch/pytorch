...
static void index_select_add(
  ...
  for (int64_t i = 0; i < numel; i++) {
    axpy<T>(
        ddim,
        1,
        src_data + ddim * select_indices_data[i],
        1,
        output_data + ddim * add_indices_data[i],
        1);
  }
}

static void make_bag_size(
    const Tensor& offsets,
    const Tensor& indices,
    const int64_t mode,
    Tensor& bag_size) {
...
std::tuple<Tensor, Tensor, Tensor, Tensor> embedding_bag_cpu_max(
    ...
    const Tensor& offsets) {
  auto max_indices =
      at::zeros(indices.type(), {offsets.size(0), weight.size(1)});
...
std::tuple<Tensor, Tensor, Tensor, Tensor> embedding_bag_cpu(
    const Tensor& weight,
    const Tensor& indices__,
...
#pragma omp parallel for if (numel > 1000)
    ...
    auto nonempty_max_indices =
        max_indices_.index_select(0, bag_size_.nonzero().view(-1));
