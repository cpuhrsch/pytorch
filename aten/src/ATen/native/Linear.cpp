...
// sumproduct_pair computes `(left*right).sum(sumdims)` by means of permutation
// and batch matrix multiplication its main purpose is to provide a pairwise
...
Tensor einsum(std::string eqn, TensorList tensors) {
  ...
  std::string in_eqn;
  size_t pos;
  // The equation is given in terms of single lowercase letters ('a'..'z') and
  // potentially an ellipsis. Internally, we represent it using indices from 0
  // to num_total_dimensions, with each letter mapped to an index and the
  // ellipsis ('...') being mapped to a number of consequtive indices. The
  // mapping of letters to internal indices is given in letter_mapping. A value
  // of -1 means that the letter has not been assigned an index yet (because it
  // has not been seen). The ellipsis is defined by first_ell_idx (the first
  // index) and num_ell_idxes (the number of indices). A value of -1 for
  // num_ell_idxes specifies that we have not seen an ellipsis yet. Note: The
  // internal indices are NOT the dimensions used internally. There is a mapping
  // to them below.

  std::array<std::int64_t, number_of_letters>
      letter_mapping; // map letter to internal (numerical) label
  letter_mapping.fill(-1);
  int64_t num_ell_idxes = -1;
  int64_t first_ell_idx = 0;

  // The internal representation of the left hand side fo the equation (with
  // ellipsis expanded) is stored in input_op_idxes. For each operand, we have a
  // vector mapping each dimension to an internal index. We also keep track of
  // the number of occurrences for each letter (to infer a right hand side if
  // not given) and of the last occurence of each index.
  std::vector<std::vector<int64_t>>
      input_op_idxes; // the parsed operand indices
  std::array<std::int64_t, number_of_letters>
      num_letter_occurrences; // number of occurrence in the equation of this
                              // letter
  num_letter_occurrences.fill(0);
  std::vector<std::int64_t>
      last_idx_occurrence; // the last operator (left to right) using this index

  if ((pos = eqn.find("->")) != std::string::npos) { // check whether we have a
                                                     // right hand side. in_eq
  ...
  // next we parse in_eq (the left hand side) by iterating. It is a string of
  // comma separated terms per index
  int64_t operand = 0;
  ...
  while (!eqn_stream.eof()) {
    std::getline(
        eqn_stream, term, ','); // term = string with indices of current term
    AT_CHECK(
        (int64_t)tensors.size() > operand,
        "more operands in equation than tensors"); // we cannot have a longer
                                                   // equation than operands. We
                                                   // need to check here before
                                                   // we use the dimension

    int64_t ell_char_count =
        0; // handling of ellipsis '...' is a bit tedious, we count the '.'
    // if there is an ellipsis, the number of dimensions it represents must be
    // total dim - letter dimensions
    int64_t candidate_num_ell_idxes = tensors[operand].dim() - term.size() + 3;
    int64_t dims_in_term = 0; // dimensions we have seen
    std::vector<int64_t> current_op_idxes; // mapping of operand dimensions to
    ...
    for (auto& c : term) { // c = character with a single letter or '.'
      if (c == '.') {
        ...
        AT_CHECK(
            ell_char_count <= 3,
            "can only have '.' in one ellispis '...' in term ",
            operand,
            " of the equation");
        if (ell_char_count == 3) { // this completes the ellipsis
          if (num_ell_idxes == -1) { // if we have not seen an ellipsis before,
                                     // keep track of indices and size
            ...
            AT_CHECK(
                candidate_num_ell_idxes == num_ell_idxes,
                "ellipsis must represent ",
                num_ell_idxes,
                " dimensions in all terms");
          }
          for (int64_t i = 0; i < num_ell_idxes;
               ++i) { // map ellipsis dimensions in operand to indices
        ...
        AT_CHECK(
            (ell_char_count == 0) || (ell_char_count == 3),
            "'.' must only occur in ellipsis, operand ",
            operand);
        AT_CHECK(
            ('a' <= c) && (c <= 'z'),
            "only lowercase letters a-z allowed as indices");
        int64_t letter_num =
            c - 'a'; // letter_num  = position in letter_mapping
        if (letter_mapping[letter_num] ==
            -1) { // new letter, add internal index and mapping
    ...
    AT_CHECK(
        dims_in_term == tensors[operand].dim(),
        "dimension mismatch for operand ",
        operand,
        ": equation ",
        dims_in_term,
        " tensor ",
  ...
  // in the check below, we need ==, but > is captured above, so the error
  // message can be specific that it is <.
  AT_CHECK(
      (int64_t)tensors.size() == operand,
      "more tensors than operands in equation");

  // the following parses or infers output (right hand side)
  // it also assigns the idxes_to_preprocessed_dims (index -> dimension in
  // preprocessed / output tensors) for the output indices. -1 means that the
  // index has not been assigned a dimension yet
  std::vector<int64_t> idxes_to_preprocessed_dims(
      num_total_idxes,
      -1); // the position of the index in the tensor dimensions
  ...
  if (pos != std::string::npos) { // parse the user provided right hand side
    ...
    for (auto& c : eqn.substr(pos + 2)) {
      if (c == '.') { // '.' as part of ellipsis
        ...
        AT_CHECK(
            ell_char_count <= 3,
            "can only have '.' in one ellispis '...' in right hand side of the equation");
        if (ell_char_count == 3) { // ellipsis complete
          AT_CHECK(
              num_ell_idxes >= 0,
              "ellipsis '...' may only appear in right hand side if it does in left hand side");
        ...
        AT_CHECK(
            (ell_char_count == 0) || (ell_char_count == 3),
            "'.' must only occur in ellipsis in the right hand side");
        AT_CHECK(
            ('a' <= c) && (c <= 'z'),
            "only lowercase letters a-z allowed as indices");
        int64_t letter_num = c - 'a';
        AT_CHECK(
            idxes_to_preprocessed_dims[letter_mapping[letter_num]] == -1,
            "index ",
            c,
            "occurs twice in output");
        idxes_to_preprocessed_dims[letter_mapping[letter_num]] =
            num_output_dims;
        num_output_dims++;
      }
    }
  ...
  // now we assign the idxes_to_preprocessed_dims (index -> dimension in
  // preprocessed / output tensors) for the non-output indices - those that are
  // eventually summed over
  ...
  // - permute the dimensions to match the order given by
  // idxes_to_preprocessed_dims
  // - unsqueeze to create all dimensions for each index in each tensor where
  // they are missing we also check that sizes match after this, all operands
  // will have compatible shapes (i.e. all dimensions are aligned are
  // broadcastable)
  std::vector<Tensor> preprocessed_operands;
  std::vector<std::int64_t> size_of_dims(
      num_total_idxes, -1); // keep track of sizes for each index, -1 means we
                            // have not seen a size yet
  for (int64_t op = 0; op < (int64_t)tensors.size(); op++) {
    auto preprocessed_op = tensors[op];
    std::vector<int64_t> idx_to_dim(
        num_total_idxes, -1); // the dimension which the index refers to in the
                              // original tensor, -1 means it does not appear
    std::vector<int64_t>& current_op_input_idxes = input_op_idxes[op];
    int64_t dim = 0; // there are two dimension indices: dim is after taking
                     // diagonals, i is in input
    for (size_t i = 0; i < current_op_input_idxes.size(); i++) {
      ...
      if (idx_to_dim[dim_out] == -1) { // first appearance
        ...
        if (size_of_dims[idx] == -1) { // keep track of sizes
          ...
          AT_CHECK(
              size_of_dims[idx] == preprocessed_op.size(dim),
              "size of dimension does not match previous size, operand ",
              op,
              ", dim ",
              i);
        }
        dim++;
      } else { // duplicate dimension in tensor --> take diagonal of
               // idx_to_dim[dim_out] and dim and put the diagonal dimension to
               // idx_to_dim[dim_out]
        AT_CHECK(
            size_of_dims[idx] == preprocessed_op.size(dim),
            "size of dimension does not match previous size, operand ",
  ...
  for (int64_t i = 1; i < (int64_t)preprocessed_operands.size(); i++) {
    ...
    result = at::native::sumproduct_pair(
        result, preprocessed_operands[i], sum_dims, true);
  }
...
// its main purpose is to unify the computations in bilinear and
// bilinear_backward
Tensor _trilinear(
    const Tensor& i1_,
    const Tensor& i2_,
  ...
  AT_CHECK(
      (unroll_dim >= 0) && (unroll_dim < total_dim),
      "unroll_dim must be in [0,",
      total_dim - 1,
      "]");
  ...
  if (!sumdim[unroll_dim]) {
    for (int64_t k = 0; k < unroll_size; k++) {
      Tensor buf = at::native::sumproduct_pair(
          i1.narrow(unroll_dim, k * slicemul1, 1),
          i2.narrow(unroll_dim, k * slicemul2, 1),
          sum_dims_12,
          true);
      buf = at::native::sumproduct_pair(
          buf, i3.narrow(unroll_dim, k * slicemul3, 1), sum_dims_23, true);
...
Tensor bilinear(
  ...
  AT_CHECK(
      ...
      "bilinear(): input dimensions do not match: got ",
      input1.dim(),
      " and ",
      input2.dim());
  for (int64_t i = 0; i < input1.dim() - 1; i++) {
    AT_CHECK(
        ...
        "bilinear(): input batch dimensions do not match at dim ",
        i,
        ": got ",
        input1.size(i),
        " and ",
