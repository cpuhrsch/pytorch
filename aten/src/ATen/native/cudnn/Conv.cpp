...
static void check_args(
  ...
  if (args.size() > expected_size) {
    std::stringstream ss;
    ss << "Too many " << arg_name << " values (" << args.size()
       << ") supplied, expecting " << expected_size
       << " (while checking arguments for " << c << ")";
    throw std::runtime_error(ss.str());
  } else if (args.size() < expected_size) {
    std::stringstream ss;
    ss << "Not enough " << arg_name << " values (" << args.size()
       << ") supplied, expecting " << expected_size
       << " (while checking arguments for " << c << ")";
    throw std::runtime_error(ss.str());
  }
...
static_assert(
    std::is_pod<ConvolutionParams>::value,
    "ConvolutionParams not POD");
...
struct ParamsEqual {
  bool operator()(const ConvolutionParams& a, const ConvolutionParams& b)
      const {
...
size_t getMaxWorkspaceSize(
  ...
  THCudaCheck(THCudaMemGetInfoCached(
      state, &free_gpu_mem, &total_gpu_mem, &max_block_size));
...
struct algorithm_search<cudnnConvolutionFwdAlgo_t> {
  ...
  static BenchmarkCache<algo_t>& cache() {
    return fwd_algos;
  }
  ...
  static perf_t findAlgorithm(const ConvolutionArgs& args) {
    ...
    return getBestAlgorithm(
        perf_results.get(), args.params.deterministic, perf_count);
  }
...
struct algorithm_search<cudnnConvolutionBwdFilterAlgo_t> {
  ...
  static perf_t findAlgorithm(const ConvolutionArgs& args) {
    ...
    CUDNN_CHECK(cudnnFindConvolutionBackwardFilterAlgorithmEx(
        ...
        ws.data,
        ws.size));
    return getBestAlgorithm<perf_t>(
        perf_results.get(), args.params.deterministic, perf_count);
  }
...
// In-place!
void cudnn_convolution_add_bias_(
    CheckedFrom c,
...
Tensor cudnn_convolution_forward(
  ...
  auto output_t = input->type().tensor(conv_output_size(
...
Tensor cudnn_convolution_backward_weight(
  ...
  convolution_shape_check(
      c, input, grad_weight, grad_output, padding, stride, dilation, groups);
...
Tensor cudnn_convolution_backward_bias(const Tensor& grad_output_t) {
  TensorArg grad_output{grad_output_t, "grad_output", 1};
  setCuDNNStreamToCurrent();

  auto grad_bias_t =
      grad_output->type().tensor({grad_output->size(output_channels_dim)});
