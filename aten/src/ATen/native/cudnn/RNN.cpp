...
struct RNNDescriptorParams {
  ...
  RNNDescriptor descriptor(
      ...
      DropoutDescriptor&& dropout_desc) const {
...
get_parameters(
    ...
    const Tensor& weight_buf) {
  auto cudnn_methods = {cudnnGetRNNLinLayerMatrixParams,
    ...
    for (auto cudnn_method : cudnn_methods) {
      for (int64_t linear_id = 0; linear_id < num_linear_layers; linear_id++) {
        ...
        if (linear_id == 0 || linear_id == num_linear_layers / 2) {
          ...
          // weight_buf.
          Tensor param = weight_buf.type().tensor().set_(
...
std::vector<int64_t> _hidden_size(
  ...
  return {rnn.num_layers * rnn.num_directions(),
          tensors.mini_batch,
          rnn.hidden_size};
}
...
Tensor _cudnn_rnn_flatten_weight(
  ...
  std::tie(params_arr, params_stride0) =
      get_parameters(handle, rnn, rnn_desc, x_desc, w_desc, weight_buf);
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor> _cudnn_rnn(
  ...
  if (!weight_buf.defined()) {
    auto num_weights = get_num_weights(
        handle, descs.rnn_desc, descs.x_descs[0], fn.rnn.datatype);
  ...
  if (cx.defined() && !cx.sizes().equals(hidden_size)) {
    std::ostringstream oss;
    oss << "Expected cell size " << IntList{hidden_size} << ", got "
        << cx.sizes();
    throw std::runtime_error(oss.str());
  }
...
std::tuple<Tensor, Tensor, Tensor> _cudnn_rnn_backward_input(
  ...
  auto y = output;
  auto w = weight_buf;
  auto dx = input.type().tensor(
      input.sizes()); // TODO: more compact way of saying this
  auto dhy = grad_hy.contiguous().view(hidden_size);
  auto dcy =
      grad_cy.defined() ? grad_cy.contiguous().view(hidden_size) : Tensor();
  ...
  if (!input.sizes().equals(input_size)) {
    std::ostringstream oss;
    oss << "Expected input size " << IntList{input_size} << ", got "
        << input.sizes();
    throw std::runtime_error(oss.str());
  }
  if (!output.sizes().equals(output_size)) {
    std::ostringstream oss;
    oss << "Expected output size " << IntList{output_size} << ", got "
        << output.sizes();
    throw std::runtime_error(oss.str());
  }
  if (hx.defined() && !hx.sizes().equals(hidden_size)) {
    std::ostringstream oss;
    oss << "Expected hidden size " << IntList{hidden_size} << ", got "
        << hx.sizes();
    throw std::runtime_error(oss.str());
  }
  if (cx.defined() && !cx.sizes().equals(hidden_size)) {
  ...
  if (dhy.defined() && !dhy.sizes().equals(hidden_size)) {
    std::ostringstream oss;
    oss << "Expected d_hidden size " << IntList{hidden_size} << ", got "
        << dhy.sizes();
    throw std::runtime_error(oss.str());
  }
  if (dcy.defined() && !dcy.sizes().equals(hidden_size)) {
    std::ostringstream oss;
    oss << "Expected d_cell size " << IntList{hidden_size} << ", got "
        << dcy.sizes();
    throw std::runtime_error(oss.str());
  }
...
std::vector<Tensor> _cudnn_rnn_backward_weight(
  ...
  return grad_weight_arr; // stride is known from call site (and also
...
std::tuple<Tensor, Tensor, Tensor, std::vector<Tensor>> _cudnn_rnn_backward(
    ...
    const Tensor& reserve,
    std::array<bool, 4> output_mask) {
  auto grad_output = grad_output_r.defined() ? grad_output_r
                                             : output.type().zeros_like(output);
  auto grad_hy = grad_hy_r.defined() ? grad_hy_r : hx.type().zeros_like(hx);
  auto grad_cy = cx.defined()
      ? (grad_cy_r.defined() ? grad_cy_r : cx.type().zeros_like(cx))
      : grad_cy_r;
