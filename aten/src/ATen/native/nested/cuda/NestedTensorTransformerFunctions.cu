...
template <typename T>
__global__
void softmax_kernel(T* qk_buf_, const int64_t seq_lens, const int batch_size, const int head_num, const int seq_len, 
  const T scalar)
{
    int batch_id = blockIdx.x / head_num;
    int offset_start = offsets[batch_id];
    int offset_end = offsets[batch_id + 1];
    int seq_len = offset_end - offset_start;
    int qk_offset = offset_start;
    ...
    for(int i = 0; i < seq_len; ++i)
    {
      ...
      float tmp = threadIdx.x < seq_len ? (float)(qk * (float)scalar): -1e20f;
